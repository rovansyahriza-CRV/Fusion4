-- =====================================================================================
-- FITUR: BYPASS TIME LIMIT UNTUK KARYAWAN KUALIFIKASI "Security"
-- Tanggal: 2026-09-25
-- =====================================================================================
-- Karyawan dengan karyawanTbl."Kualifikasi" = 'Security' (case-insensitive) absen tanpa
-- terikat window jam (Masuk1/Istirahat/Masuk2/Pulang) yang berlaku buat karyawan reguler.
-- GPS/radius lokasi TETAP dicek seperti biasa di client (find_nearest_location) sebelum
-- RPC ini dipanggil -- yang di-skip cuma validasi WAKTU-nya.
--
-- Model slot buat Security: 2 slot per hari saja.
--   - Scan pertama hari itu  -> JamMasuk1 (Status OPEN), jam berapa pun.
--   - Scan kedua hari itu    -> JamPulang (Status CLOSED), jam berapa pun.
--   - Scan ketiga dst        -> ditolak, sudah Masuk & Pulang hari ini.
--
-- Basis function: versi LIVE di Supabase (bukan file migrasi lokal lain yang sudah basi),
-- diambil via pg_get_functiondef sebelum diedit -- sudah termasuk fix LEFT JOIN timeLimitTbl
-- dan cek Cuti APPROVED yang belum sempat disalin ke file lokal manapun.
-- =====================================================================================
...
BEGIN
    ...
    -- 5. Ambil Lokasi ID & TimeLimit yang sesuai (tetap diambil walau Security -- gak dipakai
    --    kalau ternyata Security, tapi gak ganggu apa pun karena cuma dibaca)
